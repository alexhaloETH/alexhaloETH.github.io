...
function MissionFormModal({
  ...
  onSubmit,
  onDelete,
  forceCheckboxRecurrenceDaily = false,
  modalClassName,
  namePlaceholder = 'e.g., 20 pushups, Do laundry, Go to gym',
}) {
  const [name, setName] = useState(mission?.name || '');
  const [description, setDescription] = useState(mission?.description || '');
  const [recurrenceType, setRecurrenceType] = useState(mission?.recurrenceType || 'daily');
  ...
  const handleSave = () => {
    ...
    const parsedTarget = targetAmount.toString().trim() ? Number(targetAmount) : null;

    const payload = {
      ...(mission || {}),
      name: name.trim(),
      description: description.trim(),
      recurrenceType: forceCheckboxRecurrenceDaily && missionType === 'checkbox'
        ? 'daily'
        : recurrenceType,
      missionType,
