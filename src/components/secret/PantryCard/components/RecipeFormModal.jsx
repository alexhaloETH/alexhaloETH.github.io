...
function RecipeFormModal({
  ...
  const {
    ...
    buildPayload,
  } = useRecipeForm({ initialRecipe, existingRecipes });

  const handleSubmit = (e) => {
    e.preventDefault();
    const payload = buildPayload({ preserveStepMedia });
    if (!payload) return;
    onSubmit(payload);
  };
        ...
        <form onSubmit={handleSubmit} id="recipe-form" className="modal-body recipe-form">
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input
                  type="range"
                  value={alexScore}
                  onChange={(e) => setAlexScore(Number(e.target.value))}
                  min="1"
                  max="10"
                  step="1"
                />
                <div className="score-display">{alexScore}/10</div>
              </div>
            </div>
          </div>
          ...
          <div className="form-section">
            ...
            <textarea
              ...
              placeholder="Add personal notes about this recipe (e.g., substitutions you've made, tips, variations...)"
              rows="4"
            />
          </div>
        </form>
